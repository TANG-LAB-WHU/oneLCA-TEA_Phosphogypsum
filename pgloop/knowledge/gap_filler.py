...
class GapFiller:
    ...
    def add_reference_data(
        self, process_name: str, parameters: Dict[str, float], metadata: Dict = None
    ) -> None:
        """
        ...
        """
        entry = {"name": process_name, "parameters": parameters, "metadata": metadata or {}}
        self.reference_data.append(entry)

    def find_similar_processes(
        self, known_parameters: Dict[str, float], n_neighbors: int = 5
    ) -> List[Dict]:
        """
        ...
        for dist, idx in zip(distances[0], indices[0]):
            ...
            results.append({"process": valid_refs[idx], "similarity": similarity, "distance": dist})

        return results

    def predict_by_similarity(
        self, known_parameters: Dict[str, float], target_parameter: str, n_neighbors: int = 5
    ) -> PredictionResult:
        """
    ...
    def train_regression_model(self, target_parameter: str, feature_parameters: List[str]) -> float:
        """
    ...
    def predict_by_regression(
        self, known_parameters: Dict[str, float], target_parameter: str
    ) -> PredictionResult:
        """
    ...
    def fill_gaps(
        self, partial_data: Dict[str, float], target_parameters: List[str], method: str = "auto"
