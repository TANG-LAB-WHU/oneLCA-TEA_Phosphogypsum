...
class APIConnector:
    ...
    def search_openalex(
        self, query: str, filters: Optional[Dict] = None, per_page: int = 25
    ) -> List[Dict]:
        """
        ...
        response = requests.get(
            f"{source.base_url}/works", params=params, headers={"Accept": "application/json"}
        )
    ...
    def search_phosphogypsum_literature(
        self, keywords: List[str] = None, year_from: int = 2015, year_to: int = 2025
    ) -> List[Dict]:
        """
    ...
    def get_open_access_pdf(self, doi: str) -> Optional[str]:
        ...
        response = requests.get(f"{source.base_url}/{doi}", params={"email": email})
    ...
    def cache_response(self, source: str, query: str, data: Any) -> None:
    ...
    def get_cached_response(self, source: str, query: str) -> Optional[Any]:
...
def main():
    ...
    results = connector.search_phosphogypsum_literature(
        keywords=["LCA", "life cycle assessment"], year_from=2020
    )
