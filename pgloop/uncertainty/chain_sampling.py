...
class MCMCResult:
    ...
    def get_posterior_mean(self) -> Dict[str, float]:
        ...
        return {
            name: float(np.mean(self.samples[:, i]))
    ...
    def get_posterior_std(self) -> Dict[str, float]:
        ...
        return {
            name: float(np.std(self.samples[:, i]))
            for i, name in enumerate(self.parameter_names)
        }
...
class HamiltonianMC(BaseMCMC):
    ...
    def sample(
        ...
        for i in range(total_iterations):
            # Sample momentum
            p = self.rng.multivariate_normal(
                np.zeros(self.n_params), self.mass_matrix
            )
        ...
        for i in range(total_iterations):
            for name in self.parameter_names:
                if name in self.conditional_samplers:
                    ...
                    current_state[idx] = self.conditional_samplers[name](
                        current_state, self.rng
                    )
