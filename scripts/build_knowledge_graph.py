...
try:
    from pgloop.knowledge import RAGANYTHING_AVAILABLE, RAGAnythingEngine
except ImportError:
...
def step1_parse_pdfs(parser_type: str = "pymupdf", limit: int = None):
    ...
    print(f"\n{'=' * 40}")
    print(
        f"Parsing Complete: {success_count} success, {skipped_count} skipped, {error_count} errors"
    )
...
def step2_build_rag_index(limit: int = None, engine: str = "lightrag"):
    ...
    if engine == "raganything":
        ...
        results = rag.process_documents_from_directory(
            directory=UNPARSED_DIR, pattern="*.pdf", limit=limit
        )
    else:
        # LightRAG uses pre-parsed markdown
        results = rag.add_documents_from_directory(
            directory=PARSED_DIR, pattern="*.md", limit=limit, transcribe_images=True
        )
...
def step3_extract_structured_data(limit: int = None, engine: str = "lightrag"):
    ...
    if engine == "raganything":
        # RAGAnything stores parsed files in:
        # raganything_db/parsed/{doc_name}/hybrid_auto/{doc_name}.md
    ...
    for i, md_path in enumerate(md_files, 1):
        ...
        if output_path.exists():
            ...
            try:
                ...
                print("    (Failed to load existing file, re-extracting...)")
...
def step4_build_knowledge_graph():
    ...
    for json_path in json_files:
        try:
            ...
            source_id = kg.add_source(title=json_path.stem.replace("_extracted", ""), year=2024)
            stats["sources"] += 1
...
def run_pipeline(
    steps: list, parser_type: str = "pymupdf", limit: int = None, engine: str = "lightrag"
):
    ...
    if "parse" in steps or "all" in steps:
        ...
        if engine == "raganything":
            print(
                "\n[SKIP] Explicit Step 1 (Parse) is skipped because "
                "RAGAnything handles parsing internally."
            )
        else:
