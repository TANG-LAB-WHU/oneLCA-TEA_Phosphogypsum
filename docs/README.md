...
## 📁 DataHub Directory Hierarchy

The extraction pipeline reads from and writes to the structured `datahub/` folder hierarchy. Developers must follow this directory structure:
...
├── interim/
│   └── papers/
│       └── parsed/            <-- Markdown outputs generated by PDFParser (Docling/MinerU/PyMuPDF)
└── processed/
...
**1. Drop raw papers** (PDF format) into `datahub/raw/papers/unparsed/`.
